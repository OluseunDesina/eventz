...
namespace web_api_eventz.Repository
{
    public class TransactionRepository : ITransactionRepository
    {
        ...
        public async Task<TicketHistory?> BuyTicket(TicketHistory ticketHistory)
        {
            ...
            if (ticket.Quantity < ticketHistory.Quantity || eventExist == false || ticket == null || ticket.EventID != ticketHistory.EventId)
            {
                return null;
            }
            var amount = ticket.Price * ticketHistory.Quantity;
            if (amount < ticketHistory.TotalAmount)
            {
                return null;
            }
