...
namespace web_api_eventz.Repository
{
    public class TransactionRepository : ITransactionRepository
    {
        ...
        public async Task<TicketHistory?> BuyTicket(TicketHistory ticketHistory)
        {
            ...
            if (!await ValidateTicketAndEvent(ticketHistory))
            {
                return null;
            }

            await _context.TicketHistorys.AddAsync(ticketHistory);
            ticketHistory.Code = GenerateEncodedString(ticketHistory);
            ticket.Quantity -= ticketHistory.Quantity;
            ticket.TicketsSold += ticketHistory.Quantity;

            await _ticketRepo.EditTicket(ticket.Id, ticket);
            await _context.SaveChangesAsync();
            return ticketHistory;
        }

        private async Task<bool> ValidateTicketAndEvent(TicketHistory ticketHistory)
        {
            var ticket = await _ticketRepo.GetTicketById(ticketHistory.TicketId);
            ...
            ticket != null &&
            ticket.Quantity >= ticketHistory.Quantity &
            (ticket.Price * ticketHistory.Quantity) <= ticketHistory.TotalAmount &&
            ticket.EventID == ticketHistory.EventId;
        }

        private string GenerateEncodedString(TicketHistory ticketHistory)
        {
            var obj = new
            {
